...
if(isset($_POST["login"]))
{
		...
		{
			$result = mysql_query("SELECT * FROM admin WHERE login_id='$_POST[login_id]' AND password='$_POST[password]'");
			//echo "aaa".mysql_num_rows($result);
			if(mysql_num_rows($result) == 1)
			{
				...
				$dt= date("Y-m-d h:i:s");
				mysql_query("UPDATE admin SET lastlogin='$dt' where admin_id='$_SESSION[admin_id]'");
		...
		{
			$result = mysql_query("SELECT * FROM doctor WHERE login_id='$_POST[login_id]' AND password='$_POST[password]'");
			echo "aaa".mysql_num_rows($result);
			if(mysql_num_rows($result) == 1)
			{
				while($mqres = mysql_fetch_array($result))
				...
				$dt= date("Y-m-d h:i:s");
				mysql_query("UPDATE doctor SET last_login='$dt' where doc_id='$_SESSION[doc_id]'");
