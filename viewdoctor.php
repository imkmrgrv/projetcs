...
if(isset($_POST[search]))
{
	if($_POST[group] == "Name")
	{
	$resultd = mysql_query("SELECT * FROM  doctor WHERE  doc_name LIKE  '%$_POST[docname]%'");
	}
	else if($_POST[group] == "Doctor ID")
	{
	$resultd = mysql_query("SELECT * FROM  doctor WHERE doc_id LIKE '$_POST[docname]'");		
	}
	else if($_POST[group] == "Mobile No")
	{
	$resultd = mysql_query("SELECT * FROM  doctor WHERE mobile LIKE '$_POST[docname]'");		
	}
}
else if($_GET[act] =="delete")
{ 
	mysql_query("DELETE FROM doctor where doc_id='$_GET[docid]'");
...
<select name="group" id="group">
  ...
  $selbr= mysql_query("SELECT * FROM branch where branch_id = '$row[branch_id]'");
  $recbr = mysql_fetch_array($selbr);
