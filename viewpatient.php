...
if(isset($_POST[search]))
{
	...
	{
	$result = mysql_query("SELECT * FROM  patient WHERE  pat_name LIKE  '%$_POST[patname]%'");
	}
	else if($_POST[group] == "Patient ID")
	{
	$result = mysql_query("SELECT * FROM  patient WHERE pat_id=  '$_POST[patname]'");		
	}
	else if($_POST[group] == "Contact No")
	{
	$result = mysql_query("SELECT * FROM  patient WHERE contact=  '$_POST[patname]'");		
	}
}
else if($_GET[act]==delete)
{
	mysql_query("DELETE FROM patient where pat_id='$_GET[patid]'");
